...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Samplify") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Samplify", DisplayName = "Samplify API", Resources = { "Samplify" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Samplify_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Samplify_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // React SPA (password grant — geliştirme için; üretimde secret’ı güçlendirin)
        var reactClientId = configurationSection["Samplify_React:ClientId"];
        if (!reactClientId.IsNullOrWhiteSpace())
        {
            var reactSecret = configurationSection["Samplify_React:ClientSecret"];
            var reactScopes = new List<string>(commonScopes) { "offline_access" };
            await CreateApplicationAsync(
                name: reactClientId!,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Samplify React",
                secret: reactSecret,
                ...
                scopes: reactScopes,
                clientUri: configurationSection["Samplify_React:RootUrl"]?.TrimEnd('/'),
                redirectUri: null,
